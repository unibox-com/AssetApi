...
class OOrganizationCabinetModel extends Model{
    protected $trueTableName = 'o_organization_cabinet';
    ...
    public function getApartmentAddressByCabinetId($cabinetId){
        ...
        $ap = D('OOrganization')->getApartmentInfo($ap['organization_id']);
        return $ap['address'];
    }
    ...
    public function getApartmentCabinetList($wh = array()){
        return $this
            ->join('cabinet on cabinet.cabinet_id = o_organization_cabinet.cabinet_id')
            ->where($wh)->select();
    }
