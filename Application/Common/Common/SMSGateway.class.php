...
class SMSGateway {
    ...
    function getDeviceId() {
        $res = $this->getDevices();
        if($res['status'] == 200 && $res['response']['success'] == 1) {
            $devices = $res['response']['result'];
            if($devices['total'] > 0) {
                $devicesArr = $devices['data'];
                $lastSeenArr = [];
                foreach($devicesArr as $d) {
                    $lastSeenArr[] = $d['last_seen'];
                }

                array_multisort($lastSeenArr, SORT_DESC, $devicesArr);
                return $devicesArr[0]['id'];
            }
        } else {
            return false;
        }
    ...
    function sendMessageToNumber($to, $message, $device=null, $options=[]) {
        $device = $device ? : $this->getDeviceId();
        if(empty($device)) {
            return false;
        }
