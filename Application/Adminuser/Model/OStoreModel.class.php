    ...
    public function getStoreArr($wh = array(), $fieldArr = ['storeId', 'courierCompanyName', 'pickCode', 'storeTime', 'pickTime', 'cabinetId'], $formatTime=false){
        ...
        foreach($list as $store) {
            ...
            if(in_array('address', $fieldArr)) {
                if($store['cabinet_id']) {
                    $st['address'] = D('OApartmentCabinet')->getApartmentAddressByCabinetId($store['cabinet_id']);
                }
            }
    ...
    public function getStoreOrdersList($where = array(),$page,$pagesize){
      return $this
        ->join('left join member to_member on to_member.member_id = o_store.to_member_id')
				...
				->join('left join o_member_apartment on to_member.member_id = o_member_apartment.member_id')
				->join('o_apartment_cabinet on o_store.cabinet_id = o_apartment_cabinet.cabinet_id and o_apartment_cabinet.apartment_id = o_member_apartment.apartment_id')
				...
				->join('left join o_courier_company_apartment on o_store.courier_id = o_courier_company_apartment.courier_id')
				->join('left join o_courier_company on o_courier_company_apartment.company_id = o_courier_company.company_id')
        ->distinct(true)->where($where)->order('create_time desc')
        ->field('o_store.*,o_courier.courier_name,o_member_apartment.apartment_id,o_courier_company.company_name as unit_name,cabinet_box_model.model_name')
